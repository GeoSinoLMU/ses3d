...
include 'mpif.h'
    ...
    if ((adjoint_flag==0) .or. (adjoint_flag==1)) then
        ...
        if (is_source==1) then
            ...
            isx=floor((xxs-xmin)/dx)
            isy=floor((yys-ymin)/dy)
            isz=floor((zzs-(zmax_global-zmax))/dz)
